...
def get_meraki_orgs():
    ...
    orglist = requests.get(url, headers=header)
    # print(url)
    # print(header)
    orgjson = json.loads(orglist.content.decode("utf-8"))
    return orgjson
...
def get_meraki_networks(meraki_org):
    '''
...
def get_meraki_devices(meraki_net):
    '''
    Get a list of all devices associated with the specified network. Watch for 200 OK, because a network
    can have 0 devices, which will generate a 404 instead.

    :return: a dictionary with all networks that are part of the specified/derived network
    '''

    url = "https://dashboard.meraki.com/api/v0/networks/" + meraki_net + "/devices"
    #print(url)
    devlist = requests.get(url, headers=header)
    if devlist.status_code == 200:
        devjson = json.loads(devlist.content.decode("utf-8"))
    else:
        devjson = {}
        # print("Error retrieving Meraki devices:", devlist.status_code)
    return devjson
...
def manual_decode_model(orgid, netid, devsn):
    '''
    Decodes the Meraki model number into it's general type.
    ...
    ms_test = meraki.getswitchports(meraki_api_token, devsn, suppressprint=True)
    if ms_test is None or ms_test[0] == 'Feature not supported':
        mx_test = meraki.getmxperf(meraki_api_token, netid, devsn, suppressprint=True)
        if mx_test is None or mx_test[0] == 'Feature not supported':
            mr_test = getwirelessstats(meraki_api_token, netid, devsn, suppressprint=True)
            if mr_test is None or mr_test[0] == 'Feature not supported':
                mv_test = getvideolink(meraki_api_token, netid, devsn, suppressprint=True)
                if mv_test is None or mv_test[0] == 'Feature not supported':
                    mc_test = meraki.getphonedetails(meraki_api_token, netid, devsn, suppressprint=True)
                    if mc_test is None or mc_test[0] == 'Feature not supported':
                        outmodel = "unknown"
                    else:
                        outmodel = "phone"
                else:
...
# MV video link
# https://n81.meraki.com/o/p8Qg-b/manage/support/api_docs#returns-video-link-for-the-specified-camera
def getvideolink(apikey, networkid, serial, suppressprint=False):
    base_url = 'https://api.meraki.com/api/v0'
    calltype = 'MV Video Link'
    geturl = '{0}/networks/{1}/devices/{2}/videoLink'.format(str(base_url), str(networkid), str(serial))
    headers = {
...
# MX Interface Enabled
# https://n81.meraki.com/o/p8Qg-b/manage/support/api_docs#returns-the-enabled-status-of-vlans-for-the-network
def getmxvlanenabled(apikey, networkid, suppressprint=False):
    base_url = 'https://api.meraki.com/api/v0'
    calltype = 'MX VLAN Enabled'
    geturl = '{0}/networks/{1}/vlansEnabledState'.format(str(base_url), str(networkid))
    headers = {
        'x-cisco-meraki-api-key': format(str(apikey)),
        'Content-Type': 'application/json'
    }
...
# MR connection stats
# https://n81.meraki.com/o/p8Qg-b/manage/support/api_docs#aggregated-connectivity-info-for-a-given-ap-on-this-network
def getwirelessstats(apikey, networkid, serial, suppressprint=False):
    base_url = 'https://api.meraki.com/api/v0'
    calltype = 'MR Connection Status'
    geturl = '{0}/networks/{1}/devices/{2}/connectionStats'.format(str(base_url), str(networkid), str(serial))
    headers = {
        'x-cisco-meraki-api-key': format(str(apikey)),
        'Content-Type': 'application/json'
    }
...
def putssidconfig(apikey, networkid, status, name, psk, ssidnum, suppressprint=False):
    ...
    putdata = {}
    if status != "":
        if status == True or status.lower() == "true":
        ...
        putdata["authMode"] = "psk"
        putdata["encryptionMode"] = "wpa"
        putdata["wpaEncryptionMode"] = "WPA2 only"
        putdata["psk"] = psk

    # print(putdata, puturl)
